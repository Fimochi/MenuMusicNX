...
namespace tune::impl {
    ...
    void PmdmntThreadFunc(void *) {
        while (g_should_run) {
            u64 pid{}, app_tid{};
            pm::getCurrentPidTid(&pid, &app_tid);

            g_should_pause = !pm::IsHomeMenuForeground();

            if (pid && app_tid != pm::QlaunchTitleId) {
                audWrapperSetProcessMasterVolume(pid, 0, g_default_title_volume);
            }
